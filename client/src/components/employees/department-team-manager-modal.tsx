...
export default function DepartmentTeamManagerModal({ isOpen, onClose }: DepartmentTeamManagerModalProps) {
  ...
  const handleDeleteDepartment = async (code: string) => {
    if (confirm("이 부서를 삭제하시겠습니까? 관련된 팀도 함께 삭제됩니다.")) {
      try {
        await DepartmentTeamManager.removeDepartment(code);
        await loadData();
        alert("부서가 삭제되었습니다.");
      } catch (error) {
        alert(error instanceof Error ? error.message : "부서 삭제에 실패했습니다.");
      }
    }
  };
                      ...
                      {editingDepartment === dept.code ? (
                        ...
                        <>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Button size="sm" variant="outline" onClick={() => handleDeleteDepartment(dept.code)}>
                      ...
                      {editingTeam === team.code ? (
                        ...
                        <>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Button size="sm" variant="outline" onClick={() => handleDeleteTeam(team.code)}>
                              <Trash2 className="w-4 h-4" />
                            </Button>
                          </div>
                        </>
